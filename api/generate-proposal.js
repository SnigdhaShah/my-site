...
async function renderProposalPdf({ company_name, contact_name, sections }) {
  ...
  // Brand colors from styles.css: #1C2B3A (slate) and #C4775A (terra)
  const brandPrimary = rgb(0.110, 0.169, 0.227);   // slate (#1C2B3A)
  const brandAccent = rgb(0.769, 0.467, 0.353);    // terra (#C4775A)
  ...
  lastPage.drawText('snigdha.a.shah@gmail.com', {
...
// ── HTTPS agent for development (handles certificate issues) ──────────────────
...
- **HNI individuals & families** building personal legacies or foundations: ₹15k–₹50k+ annually for multi-month advisory
- **Foundations** (domestic or US-based) seeking India expansion, growth strategy, governance refresh: retainer relationships, ₹20k–₹75k+ range
- **Corporate CSR teams** wanting to move beyond compliance to strategy: ₹5k–₹20k for projects/workshops
- **Scaling nonprofits** ready to professionalize: ₹10k–₹40k for capacity-building engagements

## LEAD SCORING RULES

**HIGH** — Move forward. Strong alignment with firm strengths.
- HNI/individual building/starting a foundation with clear vision; capacity to allocate ₹15k–₹50k+ for multi-month advisory
- Foundation seeking strategy refresh, India expansion, 5-year planning, board alignment; established budget; decision-maker engaged
- Warm referral from SVP India network, existing clients, or colleagues; context provided; clear ask
- India-focused challenge; organization launching/scaling/troubleshooting work in India; understands complexity; ready to invest
- Retainer or long-term engagement (6+ months); governance/capacity building component signaled
- Budget allocated ("we have set aside X" or board approval); no haggling expected

**MEDIUM** — Qualify further. Potential fit but unclear scope/budget/timing.
- Corporate CSR team wanting to strengthen strategy/measurement; budget constrained (₹5k–₹15k); CSR mandated but not yet strategic
- Nonprofit with governance/capacity need; strong mission fit; limited budget; may require phased approach or workshop-only option
- One-off project (narrative, proposal, single workshop); clear scope; decision-maker confirmed; no signals of ongoing relationship
- US foundation interested in India but early-stage exploring; no India on-ground experience; may need education phase
- Referred without context; warm introduction from someone Snigdha knows, but no background provided; requires initial call to qualify
- Budget signals vague ("we'll need to see scope first" or "budget TBD")

**LOW** — Not the right fit. Redirect or pass.
- Student or academic inquiry (unless formal teaching program like MSW partnership)
- Generic CSR or nonprofit consulting request; no India focus; no governance angle; anyone could serve it
- Nonprofit seeking discounted/free advice ("we have limited budget" as opening)
- No clear problem statement; suggests early exploration without urgency
- Geographic misalignment (Africa, Southeast Asia, Global North); outside India expertise zone
- Cold outreach with no warm introduction; low signal of seriousness
- Compliance-only CSR ("we need to spend 2% to meet law" with no strategy appetite)

## PROPOSAL STRUCTURE

Write 4-5 sections:
1. **Understanding Your Challenge** — show you listened to their specific situation; name the problem in their words
2. **Recommended Approach** — what you would do and why; specific to their problem, not generic
...
## INSTRUCTIONS
- Write the proposal in Snigdha's voice — warm, structured, direct, specific to their situation. Use em dashes. End forward on possibility.
...
export async function generateProposal(req, res) {
  ...
  for (let turn = 1; turn <= 5; turn++) {
    ...
    try {
      ...
      console.error('Agent fetch error:', fetchError.message);
      return res.status(502).json({ error: 'Failed to connect to OpenRouter API', details: fetchError.message });
    }
    ...
    if (!assistantMessage.tool_calls || assistantMessage.tool_calls.length === 0) {
      console.log(`Agent turn ${turn}: Agent completed (no more tool calls).`);
      break;
    }
